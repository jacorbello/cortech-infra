...
- **Node labels** for K3s scheduling: use `role: core-app` for primary workloads, `role: compute` for background jobs, `role: batch-compute` for GPU/ephemeral work, `role: gpu-inference` for dedicated GPU inference workloads (taint `nvidia.com/gpu:NoSchedule`), `node-type: worker` for any worker.
