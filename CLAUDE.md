...
| cortech-node3 | 192.168.1.114 | 96 CPU, 566 GiB RAM | GPU node (Tesla T4) |
...
- **2 core-app workers** (209-210): k3s-wrk-5 (`.101`), k3s-wrk-6 (`.102`) on cortech-node3, 8 vCPU / 32 GiB each, `role=core-app`, untainted. Added because the pool was at 91-97% of memory requests on the two 8 GiB nodes; they also give core-app a third fault domain so a spread survives losing `cortech`. **Traefik runs here**, and MetalLB's L2 announcement of the ingress VIP follows it — moving Traefik onto a tainted node requires giving the MetalLB speaker a matching toleration
...
| Parley | https://parley.corbello.io | parley | Planning poker + standups. Upstream Helm chart (`ghcr.io/lets-parley/charts`), 2 replicas, shared Postgres (.83), OIDC via Keycloak realm `parley`. See `docs/runbooks/parley.md` |
...
- **Node labels** for K3s scheduling: use `role: core-app` for primary workloads (k3s-wrk-1, k3s-wrk-2, k3s-wrk-5, k3s-wrk-6), `role: batch-compute` for GPU/ephemeral work, `role: gpu-inference` for dedicated GPU inference workloads (taint `nvidia.com/gpu:NoSchedule`), `node-type: worker` for any worker.
- **Helm** is the primary K8s package manager. Key releases: `prometheus` (kube-prometheus-stack), `loki` (loki-stack), `rancher`, `harbor`, `arc-v2` (ARC controller in `arc-systems`), per-repo runner scale sets in `arc-runners` (`plotlens-runner`, `moltbot-trading-runner`, `osint-core-runner`, `cortech-infra-runner`), `traefik`, `csi-driver-nfs`. Check live state with `ssh root@192.168.1.52 "helm list -A"`.
