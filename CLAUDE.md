...
- **Node labels** for K3s scheduling: use `role: core-app` for primary workloads (k3s-wrk-1, k3s-wrk-2), `role: batch-compute` for GPU/ephemeral work, `role: gpu-inference` for dedicated GPU inference workloads (taint `nvidia.com/gpu:NoSchedule`), `node-type: worker` for any worker.
