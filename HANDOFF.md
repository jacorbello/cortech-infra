...
4. **Phase 2 T30** — after #3 done + ≥5 production posts in `published` (currently 1 — row 62 only; the new poll workflow's reconciliation is the source of truth) + 24h ArgoCD `temporal` + `postiz` Synced/Healthy window. Then `gh pr merge 18 --squash` and flip ArgoCD `targetRevision` from `outreach/phase0-phase1` to `main`/`HEAD`.
