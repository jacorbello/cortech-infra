...
**As of:** 2026-05-22 — Phase 2.1 schema cleanup + 13 followups + X investigation + 8 DEPLOYED + 12 CI drift guards landed. **Followup 12 (Slack platform_key propagation) deployed 17:14 UTC and Followup 13 (draft length budget + source attribution) deployed 17:40 UTC — first production approve click rescued, Bluesky 300-grapheme limit now pinned in the prompt.**
...
**Phase 2:** T1-T29 done + Phase 2.1 schema cleanup done (A1-A3 + B1-B7 + B5.5) + 13 followups (unified dropdown, Slack dispatch, schema test hardening, X investigation, Slack platform-picker DEPLOYED, CI drift guards, smoke fix DEPLOYED, Slack blocksUi + form dedup DEPLOYED, Slack signature fix DEPLOYED, RSS + follow.it + soft-block DEPLOYED, Assert return-shape + RSS thin-skip DEPLOYED, Slack platform_key propagation DEPLOYED, draft length + source attribution DEPLOYED). T30 is the exit gate — it can't be tagged until:
...
Issue A and Issue B from the prior handoff remain **RESOLVED** as of Followup 11 (2026-05-22 16:22 UTC). Followups 12 + 13 then rescued the first real production approve click and pinned Bluesky's 300-grapheme limit + source-attribution requirement into the drafter prompt.

Resume priorities in order:

### (a) Postiz-state poll workflow — close the honesty gap

`publish_jobs.status='sent_to_postiz'` + `outreach_items.status='published'` are currently set on Postiz HTTP 200, not on actual Bluesky publish success. Postiz uses Temporal for the platform-side post, which can silently fail asynchronously — row 72 surfaced this when Bluesky rejected the 542-char text but our DB still claimed `published`. The DB is currently capable of lying about publish state. Fix path: a new poll workflow that queries Postiz `Post.state` and only marks `outreach_items` as `published` when the state transitions to a success terminal (and flips back to `failed` / a new status on `ERROR`). Worth its own plan under `docs/superpowers/plans/` before any code lands. This is the top priority because every production post we count toward T30's "≥5 in `sent_to_postiz`" gate is unreliable until this is in place.

### (b) Threading support (multi-skeet Bluesky threads)
...
### (c) Verify Hash proper fix (task #128)

Followup 11's `code-node-return-shape-audit.js` grandfathers Workflow D's `publish-dispatcher.json:Verify Hash`. Followup 12's row-72 publish SUCCESS reinforced that n8n tolerates the array-wrap pattern in some upstream-node shapes, but the grandfather is still a known smell. Plan exists at `docs/superpowers/plans/2026-05-22-verify-hash-grandfathered-fix.md` (task #128 from this session) — pre-deploy gate (0 `ready` rows, same as B4) + synthetic publish_job smoke through the dispatcher + removal of the grandfather entry. Not blocking T30; addressing before tag is the cleaner path.

### (d) Phase 1 operational validation (user-driven, in progress)

Use the system for ≥1 week, process ≥10 real outreach items end-to-end. After Followup 13's prompt fix + the row-72 cleanup, **production-post count is 1/5** toward T30 (row 62 from T25 remains the only legitimate row). Once 10 real items are approved + dispatched (and the (a) poll workflow is honest about which of those actually published), tag Phase 1: `git tag -a outreach-phase1-shipped -m "Phase 1: approval gate end-to-end"`. This unblocks T30.
...
After (d) completes AND ≥5 production posts in `sent_to_postiz` (currently 1 — row 62 from T25; row 72 was rolled back by Followup 13 cleanup) AND ArgoCD `temporal` + `postiz` Applications stay Synced/Healthy for a full 24h window:
```bash
...
### (f) Channel onboarding (user-gated)
...
### 1. Postiz-state poll workflow (TOP PRIORITY)

`publish_jobs.status='sent_to_postiz'` + `outreach_items.status='published'` are currently set on Postiz HTTP 200, not on actual Bluesky publish success. Postiz uses Temporal under the hood and that can silently fail async — row 72 (Followup 13) is the canonical proof. Scope a new poll workflow that queries Postiz `Post.state` and only flips `outreach_items` to `published` on a success terminal. Worth its own plan in `docs/superpowers/plans/` before code lands. Every production-post count toward T30's ≥5 gate is unreliable until this lands.
...
### 3. Verify Hash proper fix (task #128)
...
### 4. Phase 1 operational validation (user-driven, in progress)

Use the system for ≥1 week, process ≥10 real outreach items end-to-end. Once done, tag Phase 1: `git tag -a outreach-phase1-shipped -m "Phase 1: approval gate end-to-end"`. This unblocks T30. Production-post count is 1/5 after the Followup 13 cleanup rolled row 72 back.

While the user is doing this, **do not deploy workflow changes** unless they explicitly confirm — n8n restart interrupts their session.

### 5. Phase 2 T30 (tag)

After step 4 completes AND ≥5 production posts in `sent_to_postiz` (currently 1 — row 62 from T25) AND ArgoCD `temporal` + `postiz` Applications stay Synced/Healthy for a full 24h window:
```bash
ssh root@192.168.1.52 "pct exec 114 -- su - postgres -c \"psql -d outreach -c 'SELECT COUNT(*) FROM publish_jobs WHERE status = '\\''sent_to_postiz'\\'';'\""
ssh root@192.168.1.52 "kubectl get applications -n argocd temporal postiz"
```
...
### 6. Channel onboarding (user-gated)
...
### 13. `publish_jobs.status='sent_to_postiz'` lies about Bluesky publish success

Surfaced by Followup 13's row 72: Workflow D's `Postiz Create Post` HTTP call succeeded (HTTP 200, `cmph6rm1m0004j0au13vc4mty` returned), the dispatcher flipped `publish_jobs.status` to `sent_to_postiz` and `outreach_items.status` to `published`, and the post then asynchronously failed inside Postiz (Bluesky API rejected with `grapheme too big (maximum 300, got 542)`, Postiz `Post.state` went `QUEUE → ERROR`). DB still said `published`. Every production-post count toward Phase 2 T30's "≥5 in `sent_to_postiz`" gate is unreliable until a poll workflow reads Postiz `Post.state` and only marks success on a Bluesky-side terminal. Tracked in TODO #1 above; worth its own plan before code lands.
...
d7cd71c feat(outreach): assert return-shape + RSS excerpt fallback + 2 CI guards
fe98348 docs(handoff): Followup 10 row + post-deploy issues for next session
...
1. **Postiz-state poll workflow** — close the honesty gap. `publish_jobs.status='sent_to_postiz'` + `outreach_items.status='published'` are currently set on Postiz HTTP 200, not on actual Bluesky publish success (Postiz uses Temporal which can silently fail async). Row 72 from Followup 13 is the canonical proof — DB said `published`, Bluesky never accepted the post. Worth its own plan in `docs/superpowers/plans/` before code lands.

2. **Threading support** — Postiz `CreatePostDto.posts[]` enables multi-skeet Bluesky threads when Followup 13's 280-char budget genuinely truncates the substance. Phase 4 enhancement; not blocking T30.

3. **Verify Hash proper fix** (task #128) — Workflow D's `publish-dispatcher.json:Verify Hash` is grandfathered in `code-node-return-shape-audit.js`. Plan at `docs/superpowers/plans/2026-05-22-verify-hash-grandfathered-fix.md`: pre-deploy gate + synthetic publish_job smoke + grandfather removal. Not blocking T30; the only remaining audit smell.

4. **Phase 1 operational validation** — ≥10 real items / ≥1 week of real usage; then tag `outreach-phase1-shipped`. Only step blocking Phase 2 tag. Production-post count is 1/5 after the Followup 13 cleanup rolled row 72 back.

5. **Phase 2 T30** — after #4 done + ≥5 production posts in `sent_to_postiz` (currently 1 — row 62 from T25) + 24h ArgoCD `temporal` + `postiz` Synced/Healthy window. Then `gh pr merge 18 --squash` and flip ArgoCD `targetRevision` from `outreach/phase0-phase1` to `main`/`HEAD`.
